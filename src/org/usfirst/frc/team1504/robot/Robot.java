...
public class Robot extends RobotBase {
	...
	public void test() {
		...
		while (isTest() && isEnabled()) {
			...
			System.out.println("Latch: " + testlatch.getRawButtonLatch(1) + "    Rising: "
					+ testlatch.getRawButtonOnRisingEdge(1) + "    Button: " + testlatch.getRawButton(1));
			Timer.delay(1.0);
		}
	}
		...
		while (true) {
			if (isDisabled()) {
				...
				while (isOperatorControl() && !isDisabled()) {
					m_ds.waitForData(150); // Blocks until we get new datas or
											// 150ms elapse
					_semaphore.newData();
					// Timer.delay(0.01);
				}
